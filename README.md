...
5. **結果を確認**
   - `result.xlsx`または`result_all_images.xlsx`ファイルを開く
   - A列：ファイルパス
...
### プログラム版による違い
| プログラム | 画像表示制限 | 出力ファイル名 |
|---|---|---|
| `main.py` | 最大3枚まで | `result.xlsx` |
| `main_improved.py` | 制限なし（全画像） | `result.xlsx` |
...
**🎉 セットアップ完了後は、targetフォルダにファイルを配置して`python main_improved.py`を実行するだけで使用できます！** 